...
testContains = contains [(Component 'a' 2), (Component 'b' 5)] 'b' -- Just 5
...
remove ((Component x p):xs) a q
    -- if existing quant. is smaller than or eq. than removal, decrease. Otherwise, take it to 0.
...
isProducible (Product a ((d,q):xs)) st
    | enoughOfCompInStorage d q st = isProducible (Product a xs) st
    | otherwise = False

enoughOfCompInStorage :: Eq a => a -> Int -> Storage a -> Bool --Iterates over storage to see if there are enough of component with desc. a
enoughOfCompInStorage d q [] = False
enoughOfCompInStorage d q ((Component x e):xs)
    | d == x = ((toInteger e) >= (toInteger q)) --fromIntegral
    | otherwise = enoughOfCompInStorage d q xs

testProducible = isProducible (Product 'a' [('x',3), ('y',6)]) [(Component 'y' 8)]